...
//// URL세션
//// DataTask
//// 3가지(코인리스트 차트 뉴스) 리퀘스트 메소드 제공
//
//class NetworkManager {
//
//    static let session = URLSession.shared
//
//    static func requestConinList(compeltion: ([Coin]) -> Void) {
...
//                print("--> success: \(response.raw.eth)")
...
//    }
//
//    static func requestCoinChartData(compeltion: ([Article]) -> Void){
...
//    }
//
//    static func requestNewsList(compeltion: ([ChartData]) -> Void){
...
extension NetworkManager {
    static func requestCoinList(compeltion: @escaping (Result<[Coin], Error>) -> Void) {
        ...
        guard let url = CoinListRequst(param: param).urlRequest().url else { return }
